...
# RESULT_DEPLOY=$(
    $TONOS_CLI -u $NETWORK call $ROOT_ADDR setDEXpairCode $ROOT_DATA --abi ../$CONTRACTS/DEXroot.abi.json --sign $CURRENT_DIR/RootB/deploy.keys.json #| grep "Transaction" | cut -c 12-) #| awk '/Transaction /')
