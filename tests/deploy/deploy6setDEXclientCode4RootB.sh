...
echo ========================================================================
echo Set DEXclientCode for Root B
...
ROOT_ADDR_FILE="$CURRENT_DIR/RootB/address.json"
...
# RESULT_DEPLOY=$(
    $TONOS_CLI -u $NETWORK call $ROOT_ADDR setDEXclientCode $ROOT_DATA --abi ../$CONTRACTS/DEXroot.abi.json --sign $CURRENT_DIR/RootB/deploy.keys.json #| grep "Transaction" | cut -c 12-) #| awk '/Transaction /')
