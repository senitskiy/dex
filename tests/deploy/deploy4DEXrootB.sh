...
ROOT_ADDR=$($TONOS_CLI -u $NETWORK genaddr ../$CONTRACTS/DEXroot.tvc ../$CONTRACTS/DEXroot.abi.json --genkey $CURRENT_DIR/RootB/deploy.keys.json --wc 0 | grep "Raw address:" | cut -c 14-)
...
RESULT_DEPLOY=$($TONOS_CLI -u $NETWORK deploy ../$CONTRACTS/DEXroot.tvc $ROOT_DATA --abi ../$CONTRACTS/DEXroot.abi.json --sign $CURRENT_DIR/RootB/deploy.keys.json --wc 0 | grep "Transaction" | cut -c 12-) 
