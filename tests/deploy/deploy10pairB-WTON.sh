...
PAIR_ADDR=$($TONOS_CLI -u $NETWORK genaddr ../$CONTRACTS/DEXpair.tvc ../$CONTRACTS/DEXpair.abi.json --genkey $CURRENT_DIR/pairB-WTON/deploy.keys.json --wc 0 | grep "Raw address:" | cut -c 14-)
...
echo { \"address\": \"$PAIR_ADDR\"} > $CURRENT_DIR/pairB-WTON/address.json
...
ROOT_A_ADDR_FILE="$CURRENT_DIR/RootWTON/address.json"
...
RESULT_DEPLOY=$($TONOS_CLI -u $NETWORK deploy ../$CONTRACTS/DEXpair.tvc $ROOT_DATA --abi ../$CONTRACTS/DEXpair.abi.json --sign $CURRENT_DIR/pairB-WTON/deploy.keys.json --wc 0 | grep "Transaction" | cut -c 12-) 
