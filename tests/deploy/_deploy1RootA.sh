...
GETNAME=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getName {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
echo Name Root: $(echo -n $GETNAME | xxd -r -p)

GETSYMBOL=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getSymbol {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
echo Symbol Root: $(echo -n $GETSYMBOL | xxd -r -p)

GETDECIMALS=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getDecimals {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
echo Decimals Root: $GETDECIMALS

GETROOTKEY=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getRootKey {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
echo Root key: $GETROOTKEY

GETTOTALSUPPLY=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getTotalSupply {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
echo Total supply: $GETTOTALSUPPLY

GETTOTALGRANTED=$($TONOS_CLI -u $NETWORK run $ROOT_ADDR getTotalGranted {} --abi ../$CONTRACTS/DEXroot.abi.json | grep "value0" | cut -c 13-)
