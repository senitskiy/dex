...
contract DEXpair is IDEXpair {
 ...
 // In the next stage it will be implemented using pair TIP-3 wallets, similar to Uniswap as proof of stake
	...
	// dwsId data
	mapping(uint256 => uint8) public rootAdwsId;
	mapping(uint256 => uint8) public rootBdwsId;
	...
	// Init function.
	constructor(address root0, address root1, uint256 rwsUint0, uint256 rwsUint1) public {
		require(msg.sender == rootDEX, 102);
		tvm.accept();
		setPairRoots(root0, root1, rwsUint0, rwsUint1);
		initStatusA = false;
		initStatusB = false;
	}
	...
	// Function to get max from rootAdwsId data
	function getMaxRootAdwsId() private view inline returns (uint256) {
		optional(uint256, uint8) rs = rootAdwsId.max();
		if (rs.hasValue()) {(uint256 number, ) = rs.get();return number;} else {return 0;}
	}

	// Function to get max from rootBdwsId data
	function getMaxRootBdwsId() private view inline returns (uint256) {
		optional(uint256, uint8) rs = rootBdwsId.max();
		if (rs.hasValue()) {(uint256 number, ) = rs.get();return number;} else {return 0;}
	}

	// Function to get pair roots max dwsId
	function getPairRootsMaxDwsId() public view returns (uint256 maxA, uint256 maxB) {
		maxA = getMaxRootAdwsId();
		maxB = getMaxRootBdwsId();
	}

	// Function to create  UINT256 for create different address for new wallets.
	function createWalletId(address rootAddr) private inline returns (uint256) {
		uint256 dwsId;
		if (rootAddr == rootA) {
			 dwsId = getMaxRootAdwsId();
			 dwsId++;
			 rootAdwsId[dwsId] = 1;
		} else if (rootAddr == rootB) {
			 dwsId = getMaxRootBdwsId();
			 dwsId++;
			 rootBdwsId[dwsId] = 1;
		}
		return dwsId;
	...
	function createDepositWallet(address rootAddr) private inline returns (bool createStatus) {
		createStatus = false;
		uint256 walletId = createWalletId(rootAddr);
	...
	// Function to create empty deposit wallet for DEX client with walletId.
	function createDepositWalletWithId(address rootAddr, uint256 walletId) private pure inline returns (bool createStatus) {
		createStatus = false;
		address creator = rootAddr;
		address owner = address(this);
		uint256 ownerUINT = owner.value;
	...
	function connect() public alwaysAccept override functionID(0x00000005) {
		...
		uint128 dexclientValue = msg.value;
		tvm.rawReserve(address(this).balance - dexclientValue, 2);
		if (!dexpairclients.exists(dexclient)){
			...
			TvmCell body = tvm.encodeBody(IDEXclient(dexclient).setPair, rootA, address(0), rootB, address(0),0,0);
	...
	// Function to connect DEX client to DEX pair only if DEX client not connected earlier.
	function connectWithId(uint256 dwsIdA, uint256 cwsIdA, uint256 dwsIdB, uint256 cwsIdB) public alwaysAccept override functionID(0x00000055) {
		address dexclient = msg.sender;
		uint128 dexclientValue = msg.value;
		tvm.rawReserve(address(this).balance - dexclientValue, 2);
		if (!dexpairclients.exists(dexclient) && !rootAdwsId.exists(dwsIdA) && !rootBdwsId.exists(dwsIdB)){
			dexpairclientKeys.push(dexclient);
			addDEXclientToQueueA(dexclient);
			addDEXclientToQueueB(dexclient);
			createDepositWalletWithId(rootA,dwsIdA);
			createDepositWalletWithId(rootB,dwsIdB);
			rootAdwsId[dwsIdA] = 1;
			rootBdwsId[dwsIdB] = 1;
			Client cc = dexpairclients[dexclient];
			cc.index = dexpairclientKeys.length;
			...
			cc.status = 0;
			dexpairclients[dexclient] = cc;
			TvmCell body = tvm.encodeBody(IDEXclient(dexclient).setPair, rootA, address(0), rootB, address(0), cwsIdA, cwsIdB);
			dexclient.transfer({ value: 0, flag: 128, body:body});
		} else {
			dexclient.transfer({ value: 0, flag: 128 });
		}
	...
	function setPairDepositWallet(address value0) public override alwaysAccept functionID(0x0000000a){
		...
		uint128 rootChange = msg.value;
		address wallet = value0;
		tvm.rawReserve(address(this).balance - rootChange, 2);
		if (root == rootA){
	...
	function createReserveWallet(address rootAddr, uint256 rwsUINT) private pure returns (bool createStatus) {
		createStatus = false;
		address creator = rootAddr;
		address owner = address(this);
		uint256 ownerUINT = owner.value;
		TvmCell body = tvm.encodeBody(IRootTokenContract(creator).deployEmptyWallet, 0x0000000b, 0, rwsUINT, ownerUINT, GRAMS_CREATE_NEWWALLET);
		creator.transfer({value:GRAMS_CREATE_ROOT, bounce:false, body:body});
		createStatus = true;
	}

	// Function for constructor to set DEX pair.
	function setPairRoots(address root0, address root1, uint256 rwsUintA, uint256 rwsUintB) private  {
		rootA = root0;
		createReserveWallet(rootA,rwsUintA);
		rootB = root1;
		createReserveWallet(rootB,rwsUintB);
	}
