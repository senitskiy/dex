...
contract DEXroot is IDEXroot {
	...
	// Grams constants
	uint128 constant public GRAMS_CREATE_DEX_CLIENT = 10 ton;
	uint128 constant public GRAMS_CREATE_DEX_PAIR = 10 ton;
	...
	function computeDEXclientAddr(uint256 pubkey) public alwaysAccept view returns (address){
		TvmCell stateInit = tvm.buildStateInit({
			contr: DEXclient,
			varInit: {rootDEX:address(this),wTONroot:wrappedTONroot,wTONwrapper:TONwrapper},
			code: codeDEXclient,
			pubkey: pubkey
		});
	...
	function createDEXclient(uint256 pubkey) public alwaysAccept returns (address deployedAddress, bool statusCreate){
		statusCreate = false;
		...
		if (!clients.exists(deployedAddress)){
	...
	function computeDEXpairAddrWithId(uint256 pubkey, uint256 userId) public alwaysAccept view returns (address value0, uint256 createId){
		createId = userId;
		TvmCell stateInit = tvm.buildStateInit({
			contr: DEXpair,
			varInit: {rootDEX: address(this), pairID: createId},
	...
	function createDEXpair(address root0, address root1, uint256 createId) public override alwaysAccept functionID(0x00000111){
